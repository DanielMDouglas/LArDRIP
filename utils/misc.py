...
def center_coords(points_tensor):

    # Reshape the tensor to calculate the mean along the second dimension (64 coordinates)
    reshaped_tensor = points_tensor.view(points_tensor.size(0), points_tensor.size(1), -1)  # Batch, 64, 3

    # Calculate the mean along the second dimension to get the center coordinate
    center_coordinates = torch.mean(reshaped_tensor, dim=1)  # Batch, 3 (x, y, z)

    # Print the center coordinates for each batch
    #for batch_idx, center_coord in enumerate(center_coordinates):
        #print(f"Batch {batch_idx + 1} center coordinate: {center_coord}")
